# Tutorial Reference (Known-Good)

This folder contains a complete ROS 2 package that already includes:

- `ros2_ws/src/ros_moos_bridge/src/ros_moos_gateway_example.cpp`
- `ros2_ws/src/ros_moos_bridge/CMakeLists.txt`
- `ros2_ws/src/ros_moos_bridge/package.xml`

Use it to compare your manual setup or as a fallback if your local package fails to build.
Use the helper publishers at repo root:

- `~/ros-moos-bridge-tutorial/publish_example_odom_once.sh`
- `~/ros-moos-bridge-tutorial/publish_example_odom.py`
